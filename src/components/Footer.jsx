...
export default function Footer({ faqWhite }) {
  return (
    <footer className="bg-gray-900 text-white py-6 sm:py-8 text-center mt-8 sm:mt-12 border-t border-gray-800 relative overflow-hidden px-2 sm:px-0">
      {/* Animasyon svg bg */}
      ...
      <div className="flex flex-col items-center gap-1 sm:gap-2 relative z-10">
        ...
        <Link
          to="/faq"
          className={`inline-block ${faqWhite ? 'text-white' : 'text-primary'} hover:underline font-semibold transition-colors duration-200 mt-1`}
        >
          Sıkça Sorulan Sorular
        </Link>
        <span className="text-xs sm:text-sm text-gray-400">&copy; {new Date().getFullYear()} Artema Mimarlık. Tüm hakları saklıdır.</span>
      </div>
